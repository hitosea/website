...
发布 hitosea.com 新版本。版本号只存在于根目录 `package.json`，发版流程就是改版本号 + 提交 + 推送。CI 会自动根据版本号构建 Docker 镜像并发布到 GHCR。
...
3. **建议下一个版本**：默认建议 **patch** 递增（`0.1.0 → 0.1.1`），同时列出 minor / major 选项供用户选择。遵循 SemVer：
   - patch：bug 修复、文档、小调整
   - minor：新增功能（向后兼容）
   - major：破坏性变更
...
回复形如：`已发布 vX.Y.Z，已推送到 origin/main。CI 将自动构建 Docker 镜像到 ghcr.io/hitosea/website:X.Y.Z` 不需要总结改动，不需要列步骤。
